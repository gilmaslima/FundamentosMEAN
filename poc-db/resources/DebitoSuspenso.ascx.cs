...
{
    public partial class DebitoSuspenso : BaseUserControl, IControlesParaDownload
    {
        #region Attribute
        ...
        {
            using (Logger Log = Logger.IniciarLog("Débitos Suspensos"))
            {
                ...
                try
                {
                    ...
                    {
                        objRetorno = contexto.Cliente.ConsultarSuspensaoPesquisaDebito(out objStatusRetorno, TradudorEnvioSPParaServico(buscarDados), pagina, qtdLinhasPagina, GuidPesquisa(), GuidUsuario());
                ...
                catch (FaultException<Servico.CSP.GeneralFault> ex)
                {
                    Log.GravarErro(ex);
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Redecard.PN.Comum.PortalRedecardException(CODIGO_ERRO, FONTE, ex);
                }
            }
        }

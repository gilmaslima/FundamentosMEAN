...
namespace Redecard.Portal.Aberto.WebParts.FiltragemDicasCategoria
{
    ...
    /// </summary>
    public partial class FiltragemDicasCategoriaUserControl : UserControlBase
    {
        #region Propriedades
        /// <summary>
        ...
        /// </summary>
        private FiltragemDicasCategoria WebPart
        {
            get
            {
                return this.Parent as FiltragemDicasCategoria;
            }
        }
            ...
            try
            {
                using (var repository = SharePointServiceLocator.GetCurrent().GetInstance<IRepository<DTODica, DicasItem>>())
                {
                    ...
                    delegate
                    {
                        ...
                        lnkVejaMais.InnerText = RedecardHelper.ObterResource("filtragemDicasCategoria_VejaMais");

                        if (dicas.TotalItens > this.ItensPorPagina)
                            lnkVejaMais.Visible = true;
                        else
                            lnkVejaMais.Visible = false;
                    });
                }
            }
            catch (Exception)

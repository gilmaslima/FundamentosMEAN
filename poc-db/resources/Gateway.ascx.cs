...
{
    /// <summary>
    /// UserControl do Box Gateway
    /// </summary>
    public partial class Gateway : BaseUserControl
    {
        #region [ Controles ]
        ...
        private Int32 Consultar(Guid guid, BuscarDados dados, Int32 pagina, Int32 tamanhoPagina)
        {
            ...
            var registros = default(List<WAExtratoServicosServico.Gateway>);
            ...
            using (Logger log = Logger.IniciarLog("Relatório de Serviços - Gateway"))
            {
                try
                {
                    using (var contexto = new ContextoWCF<HISServicoWA_Extrato_ServicosClient>())
                        registros = contexto.Cliente.ConsultarGateway(

...
namespace Redecard.PN.Extrato.SharePoint.WebParts.Relatorios
{
    ...
    public class DynamicControlsPlaceholder : PlaceHolder
    {
        ...
        /// <summary>
        /// Caminho do ASCX do controle filho
        /// </summary>
                ...
                case "UC":
                    ...
                    if (Environment.Version.Major > 1)
                    {
                        try
                        {
                            ...
                            {
                                // caso dê erro de arquivo não encontrado, pode ser que tenha ocorrido uma mudança 
                                // dos servidores no balanceamento, neste caso, não recriar o controle a partir do tipo
                                // e sim do caminho do ASCX
                                if (!String.IsNullOrEmpty(ASCXPath))
                                    control = this.Page.LoadControl(ASCXPath);
                            }
                        }
                        catch (Exception e)
                        {

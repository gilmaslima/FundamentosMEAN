...
{
    /// <summary>
    /// UserControl do Box Boleto
    /// </summary>
    public partial class Boleto : BaseUserControl
    {
        #region [ Controles ]
        ...
        private Int32 Consultar(Guid guid, BuscarDados dados, Int32 pagina, Int32 tamanhoPagina)
        {
            ...
            var registros = default(List<WAExtratoServicosServico.Boleto>);
            ...
            using (Logger log = Logger.IniciarLog("Relatório de Serviços - Boleto"))
            {
                try
                {
                    using (var contexto = new ContextoWCF<HISServicoWA_Extrato_ServicosClient>())
                        registros = contexto.Cliente.ConsultarBoleto(

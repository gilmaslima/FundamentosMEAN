...
namespace Redecard.PN.DadosCadastrais.SharePoint.Layouts.DadosCadastrais
{
    ...
    /// </summary>
    public class Concluido : ApplicationPageBaseAnonima
    {
        #region Controles
        /// <summary>
        /// 
        ...
        /// 
        /// </summary>
        protected SharePoint.ProcessoConcluido ctrlProcessoConcluido;
        /// <summary>
        /// 
        /// </summary>
        ...
        {
            //SetarMensagem("Os seus dados de acesso foram enviados para o e-mail cadastrado.");
        ...
        /// 
        /// </summary>
        private void EnviarEmailNotificacao(String novaSenha)
        {
            InformacaoUsuario _dados = InformacaoUsuario.Recuperar();

            String url = SPContext.Current.Web.Url;
            String usuario = _dados.Usuario;
            String pv = _dados.NumeroPV.ToString();
            String senha = novaSenha;

            String path = Server.MapPath("/_layouts/DadosCadastrais/MODELOS/DadosAcesso.htm");

            WebClient client = new WebClient();
            byte[] byteHtml = client.DownloadData(path);
            String _htmlBody = System.Text.Encoding.UTF8.GetString(byteHtml);
            client.Dispose();

            _htmlBody = _htmlBody.Replace("##PV##", pv);
            _htmlBody = _htmlBody.Replace("##USUARIO##", usuario);
            _htmlBody = _htmlBody.Replace("##SENHA##", senha);
            _htmlBody = _htmlBody.Replace("##URL##", url);

            SPUtility.SendEmail(SPContext.Current.Web,
                false,
                false,
                _dados.EmailEntidade,
                "Portal Rede",
                _htmlBody
                );
        }
        ...
        private void EfetuarEnvioUsuario()
        {
            try
            {
                if (ctrlProcessoConcluido.Visible)
                {
                    ...
                    {
                        EntidadeServico.Entidade entidade = _client.ConsultarDadosPV(out codigoRetorno, info.NumeroPV);

                        // definir nova senha
                        String novaSenha = this.GerarSenha();
                        this.RenovarUsuario(novaSenha);
                        this.EnviarEmailNotificacao(novaSenha);
                        this.GeraCarta(novaSenha, entidade);
                    }
                    // setar mensagem de aviso no controle
                    SetarMensagem("Os seus dados de acesso foram enviados para o e-mail cadastrado.");
                }
            }
        ...
        /// <returns></returns>
        private Int32 GeraCarta(String novaSenha, EntidadeServico.Entidade entidade)
        {
            InformacaoUsuario info = InformacaoUsuario.Recuperar();

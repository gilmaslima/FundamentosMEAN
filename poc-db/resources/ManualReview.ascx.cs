...
{
    /// <summary>
    /// UserControl do Box Manual Review
    /// </summary>
    public partial class ManualReview : BaseUserControl
    {
        #region [ Controles ]
        ...
        private Int32 Consultar(Guid guid, BuscarDados dados, Int32 pagina, Int32 tamanhoPagina)
        {
            ...
            var registros = default(List<WAExtratoServicosServico.ManualReview>);
            ...
            using (Logger log = Logger.IniciarLog("Relatório de Serviços - Manual Review"))
            {
                try
                {
                    using (var contexto = new ContextoWCF<HISServicoWA_Extrato_ServicosClient>())
                        registros = contexto.Cliente.ConsultarManualReview(

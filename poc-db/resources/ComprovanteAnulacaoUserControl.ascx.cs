using System;
using System.Web.UI;
using System.Web.UI.WebControls;
using System.Web.UI.WebControls.WebParts;
using Redecard.PN.Comum;
using Redecard.PN.Cancelamento.Sharepoint.ServiceCancelamento;
using System.Collections.Generic;
using iTextSharp.text.pdf;
using iTextSharp.text;
using System.IO;
using System.Web;
using iTextSharp.text.html.simpleparser;
using Microsoft.SharePoint;
using System.Net;
using Winnovative.WnvHtmlConvert;
using System.Text;

namespace Redecard.PN.Cancelamento.Sharepoint.WebParts.ComprovanteAnulacao
{
    public partial class ComprovanteAnulacaoUserControl : UserControlBase
    {
        #region Metodos Tela
        /// <summary>
        /// Metodo para carga inicial dos dados da tela - Evento de Chamada
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        EstabelecimentoCancelamento estabelecimento = null;
          
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!object.ReferenceEquals(base.SessaoAtual, null))
            {
                using (Logger Log = Logger.IniciarLog("Comprovante Anulação - Page Load"))
                {
                    using (ServicoPortalCancelamentoClient client = new ServicoPortalCancelamentoClient())
                    {

                        DateTime horaimpressao = DateTime.Now;
                        LblDataImpressao.Text = "Data da Consulta: " + DateTime.Now.ToString("dd/MM/yyyy") + " às " + DateTime.Now.ToShortTimeString() + "";
                        try
                        {
                            estabelecimento = client.RetornaDadosEstabelecimentoCancelamento(this.SessaoAtual.CodigoEntidade);
                        }
                        catch (Exception ex)
                        {
                            SharePointUlsLog.LogErro(ex.Message);
                            Log.GravarErro(ex);
                        }
                    }

                    if (estabelecimento == null || !estabelecimento.Centralizadora)
                    {
                        //ScriptManager.RegisterStartupScript(Page, GetType(), "key", "$('.ocultar').hide();", true);
                        // ScriptManager.RegisterStartupScript(Page, GetType(), "key", "document.getElementsByClassName('ocultar').style.display = 'none';", true);
                        ScriptManager.RegisterStartupScript(Page, GetType(), "key", "$('.ocultar').hide();", true);
                    }

                    if (!IsPostBack)
                    {
                        //Carrega popula tabela oculta de dados do estabelecimento
                        nomedoestabelecimento.Text = estabelecimento.NomeEntidade;
                        endereco.Text = estabelecimento.Endereco;
                        numestabelecimento.Text = estabelecimento.NumeroFax;
                        //dataanulacao.Text = estabelecimento.DataCriacao.ToShortDateString();
                        dataanulacao.Text = DateTime.Now.ToString("dd/MM/yyyy");

                        CarregaTabelaLista();
                        string baseURL = SPContext.Current.Web.Url + "/_layouts/RelatorioAnulacao.aspx";
                        //lnkPdf.OnClientClick = "fImpressao('" + baseURL + "'); return false;";
                    }
                }
            }
        }

        /// <summary>
        /// Método para carregar os dados do comprovante.
        /// </summary>
        private void CarregaTabelaLista()
        {
            List<ModComprovante> comprovantesValidos = (List<ModComprovante>)Session["ItensAnulamento"];
            List<ModAnularCancelamento> comprovantesSaida = (List<ModAnularCancelamento>)Session["ItensAnulamentoSaida"];

            List<ModComprovante> _comprovantes = new List<ModComprovante>();
            Dictionary<ModComprovante, Int32> _erros = new Dictionary<ModComprovante, Int32>();
            foreach (ModComprovante comprovante in comprovantesValidos)
            {
                Boolean valido = false;
                Int32 _erro = 0;
                foreach (ModAnularCancelamento comprovanteSaida in comprovantesSaida)
                {
                    if (comprovanteSaida.valorCancel.Equals(comprovante.ValorCancelamento) && (comprovante.NumCartao == comprovanteSaida.numCartao || comprovante.NumNSU == comprovanteSaida.numNsu))
                    {
                        if (Int32.Parse(comprovanteSaida.CodRetorno) == 0)
                        {
                            valido = true;
                            _comprovantes.Add(comprovante.Clone() as ModComprovante);
                        }
                        else
                        {
                            _erro = Int32.Parse(comprovanteSaida.CodRetorno);
                        }
                        break;
                    }
                }

                if (!valido)
                {
                    _erros.Add((ModComprovante)comprovante.Clone(), _erro);
                }
            }

            if (_erros.Count > 0)
            {
                StringBuilder sb = new StringBuilder();
                foreach (ModComprovante comp in _erros.Keys)
                {
                    sb.AppendLine(String.Format("Ocorreu uma falha ao anular o cancelamento {0}. Retorno: {1}<br />", comp.NumAvisoCancelamentoFormatada, _erros[comp]));
                }
                lblMsgErro.Text = sb.ToString();
                pnlErros.Visible = true;
            }

            List<ModComprovante> comprovantes = _comprovantes;

            rptDados.DataSource = comprovantes;
            rptDados.DataBind();
        }

        /// <summary>
        /// Método executado no evento de clique do botão voltar!
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void Cancelamento_Voltar_Click(object sender, System.EventArgs e)
        {
            using (Logger Log = Logger.IniciarLog("Comprovante Anulação - Voltar"))
            {
                this.Session.Remove("ItensAnulamento");
                this.Session.Remove("ItensAnulamentoSaida");

                Response.Redirect("pn_cancelamento.aspx");
            }
        }
        #endregion

        #region Metodos Acoes
        /// <summary>
        /// Método que salva os dados como PDF
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void GerarExcel_Click(object sender, EventArgs e)
        {
            using (Logger Log = Logger.IniciarLog("Comprovante Anulação - Gerar Excel"))
            {
                Response.Clear();
                Response.Buffer = true;
                Response.AddHeader("content-disposition", "attachment;filename=comprovanteanulacao.xls");
                //   HttpContext.Current.Response.ContentEncoding = System.Text.Encoding.UTF8;
                Response.ContentType = "application/vnd.ms-excel";

                Response.ContentEncoding = System.Text.Encoding.Unicode;
                Response.BinaryWrite(System.Text.Encoding.Unicode.GetPreamble());

                System.IO.StringWriter stringWrite = new System.IO.StringWriter();
                System.Web.UI.HtmlTextWriter htmlWrite = new HtmlTextWriter(stringWrite);
                rptDados.RenderControl(htmlWrite);
                Response.Write("<table>");
                Response.Write(stringWrite.ToString());
                Response.Write("</table>");
                Response.End();

                //Response.Redirect("pn_comprovanteanulacao.aspx");
            }
        }

        public static byte[] ConverteUrlToPdf(string url)
        {
            return new PdfConverter
            {
                LicenseKey = "rIedjJ2Mnp+YjJmCnIyfnYKdnoKVlZWV",
                PdfDocumentOptions =
                {
                    PdfPageSize = PdfPageSize.A4,
                    PdfCompressionLevel = PdfCompressionLevel.Normal,
                    PdfPageOrientation = PDFPageOrientation.Landscape,
                    GenerateSelectablePdf = true,
                    FitWidth = true,
                    EmbedFonts = false,
                    LiveUrlsEnabled = true,
                    JpegCompressionEnabled = true
                },
                ScriptsEnabled = true,
                ActiveXEnabled = true,
            }.GetPdfBytesFromUrl(url);
        }

        /// <summary>
        /// Método para gerar PDF.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        //protected void GerarPDF_Click(object sender, EventArgs e)
        //{
        //    PdfConverter converter = new PdfConverter();
        //    converter.LicenseKey = "rIedjJ2Mnp+YjJmCnIyfnYKdnoKVlZWV";

        //    string baseURL = SPContext.Current.Web.Url + "/_layouts/RelatorioAnulacao.aspx";
        //    byte[] file = converter.GetPdfBytesFromUrl(baseURL);
        //    Response.ContentType = "application/pdf";
        //    Response.AddHeader("content-disposition", "attachment; filename=ProtocoloAnulacao.pdf");
        //    Response.BinaryWrite(file);
        //    Response.End();


        //    //converter.SavePdfFromUrlToFile(SPContext.Current.Site.Url + "/_layouts/RelatorioAnulacao.aspx", "ComprovanteAnulacao.pdf");
        //}
        protected void GerarPDF_Click(object sender, EventArgs e)
        {
            using (Logger Log = Logger.IniciarLog("Comprovante Anulação - Gerar PDF"))
            {
                DateTime DataGeracaoPDF = DateTime.Now;
                PdfPTable table = new PdfPTable(9);
                table.TotalWidth = 900;

                Document doc = new Document(PageSize.A2);

                DateTime horaimpressao = DateTime.Now;

                string img = "";
                byte[] array = Convert.FromBase64String(img);
                iTextSharp.text.Image headerImage = iTextSharp.text.Image.GetInstance(array);

                PdfPCell header = new PdfPCell(headerImage);
                PdfPCell linha_impressao = new PdfPCell((new PdfPCell(new Phrase(new Phrase("Data da Consulta: " + horaimpressao.ToShortDateString() + " às " + horaimpressao.ToShortTimeString() + "", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 8, 1, new BaseColor(25, 60, 145))))) { Padding = 5, Border = 0, Colspan = 9, BackgroundColor = BaseColor.WHITE, HorizontalAlignment = 2 }));
                PdfPCell cell = new PdfPCell((new PdfPCell(new Phrase(new Phrase("COMPROVANTE DE ANULAÇÃO DE CANCELAMENTO DE VENDAS", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.WHITE)))) { Padding = 5, Colspan = 2, BackgroundColor = new BaseColor(25, 60, 145), HorizontalAlignment = 0 }));

                header.Colspan = 9;
                header.HorizontalAlignment = 0;
                header.Border = 0;
                cell.Colspan = 9;
                cell.HorizontalAlignment = 0; //0=Left, 1=Centre, 2=Right
                table.AddCell(header);
                table.AddCell(linha_impressao);
                table.AddCell(cell);

                table.AddCell((new PdfPCell(new Phrase(new Phrase("", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 8, 1, BaseColor.BLACK)))) { Padding = 5, Border = 0, Colspan = 9, BackgroundColor = BaseColor.WHITE, HorizontalAlignment = 0 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("Nome de Estabelecimento:" + SessaoAtual.NomeEntidade + "", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 8, 1, BaseColor.BLACK)))) { Padding = 5, Border = 0, Colspan = 9, BackgroundColor = BaseColor.WHITE, HorizontalAlignment = 0 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("Endereço: " + estabelecimento.Endereco, new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 8, 1, BaseColor.BLACK)))) { Padding = 5, Colspan = 9, Border = 0, BackgroundColor = BaseColor.WHITE, HorizontalAlignment = 0 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("Nº do Estabelecimento: " + SessaoAtual.CodigoEntidade + "", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 8, 1, BaseColor.BLACK)))) { Padding = 5, Border = 0, Colspan = 9, BackgroundColor = BaseColor.WHITE, HorizontalAlignment = 0 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("Data do Anulação:" + horaimpressao.ToShortDateString() + "", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 8, 1, BaseColor.BLACK)))) { Padding = 5, Border = 0, Colspan = 9, BackgroundColor = BaseColor.WHITE, HorizontalAlignment = 0 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 8, 1, BaseColor.BLACK)))) { Padding = 5, Border = 0, Colspan = 9, BackgroundColor = BaseColor.WHITE, HorizontalAlignment = 0 }));

                table.AddCell((new PdfPCell(new Phrase(new Phrase("     Nº Aviso do\nEstabelec.\ndo Cancel.     ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("     Nº do Estabelec.\nde Venda     ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("     Nº do Aviso de\nCancelamento       ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("     NSU/CV     ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("     Data da\nVenda     ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("     Valor de\nVenda (R$)     ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("     Tipo de\nCancel.     ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("     valor de\nCancel. (R$)     ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));
                table.AddCell((new PdfPCell(new Phrase(new Phrase("     Tipo de\nVenda     ", new iTextSharp.text.Font(iTextSharp.text.Font.FontFamily.HELVETICA, 12, 1, BaseColor.BLACK)))) { Padding = 5, BackgroundColor = new BaseColor(211, 211, 211), HorizontalAlignment = 1 }));

                List<ModComprovante> comprovantes = (List<ModComprovante>)Session["ItensAnulamento"];

                if (comprovantes != null)
                {
                    foreach (ModComprovante ItemModComprovante in comprovantes)
                    {
                        table.AddCell(Convert.ToString(ItemModComprovante.NumEstabelecimento));
                        table.AddCell(Convert.ToString(ItemModComprovante.NumEstabelecimento));
                        //table.AddCell(ItemModComprovante.NumAvisoCancel);
                        table.AddCell(ItemModComprovante.NumAvisoCancelamentoFormatada);
                        table.AddCell(ItemModComprovante.NumCartao);
                        table.AddCell(Convert.ToString(ItemModComprovante.DataTransacao.ToShortDateString()));
                        table.AddCell(Convert.ToString(ItemModComprovante.ValorTransacao));
                        table.AddCell(ItemModComprovante.TipoCancelamentoFormatado);
                        table.AddCell(Convert.ToString(ItemModComprovante.ValorCancelamentoFormatada));
                        table.AddCell(ItemModComprovante.TipoVendaFormatado);
                    }
                }

                MemoryStream ms = new MemoryStream();
                PdfWriter.GetInstance(doc, ms);
                doc.Open();
                doc.Add(table);
                doc.Close();

                byte[] file = ms.GetBuffer();
                byte[] buffer = new byte[4096];

                Response.ContentType = "application/pdf";
                Response.AddHeader("content-disposition", "attachment; filename=comprovanteanulacao.pdf");
                Response.BinaryWrite(file);
                HttpContext.Current.ApplicationInstance.CompleteRequest();

                //Response.Redirect("pn_comprovanteanulacao.aspx");
            }
        }

        #endregion
    }
}

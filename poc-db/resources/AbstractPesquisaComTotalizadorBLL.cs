...
{
    public abstract class AbstractPesquisaComTotalizadorBLL<TEnvioDTO, TRetornoDTO, TTotalizadorDTO> : RegraDeNegocioBase
    {
        ...
        /// <returns></returns>
        protected abstract RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO> ExecutarPesquisa(out StatusRetornoDTO statusRetornoDTO, TEnvioDTO envio);
        ...
        /// <returns></returns>
        public RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO> Pesquisar(out StatusRetornoDTO statusRetornoDTO, TEnvioDTO envio, int numeroPagina, int quantidadeRegistrosPorPagina, Guid guidPesquisa, Guid guidUsuarioCacheExtrato)
        {
        ...
        /// <param name="guidUsuarioCacheExtrato">Guid de usuário que será utilizado para armazenar no cache. O Guid de Usuario serve para identificar unicamente o usuário no cache.</param>
        /// <returns></returns>
        protected RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO> PesquisarComFiltro<TParam>(out StatusRetornoDTO statusRetornoDTO, TEnvioDTO envio, TParam parametrosPreFiltro, PreFiltro<TParam> preFiltro, int numeroPagina, int quantidadeRegistrosPorPagina, Guid guidPesquisa, Guid guidUsuarioCacheExtrato)
        {
            try
            {

                RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO> retorno = BuscarDadosNoServico(envio, guidPesquisa, guidUsuarioCacheExtrato, out statusRetornoDTO);
                ...
                RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO> result = new RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO>();
                result.Totalizador = ObterTotalizadores(registros, retorno.Totalizador, parametrosPreFiltro);
        ...
        // protected abstract TTotalizadorDTO ObterTotalizadores(RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO> retorno, Object parametrosPreFiltro);

        protected virtual TTotalizadorDTO ObterTotalizadores(List<TRetornoDTO> linhas, TTotalizadorDTO totalizador, Object parametrosPreFiltro)
        {
            return totalizador;
        }
        ...
        /// <returns></returns>
        private RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO> BuscarDadosNoServico(TEnvioDTO envio, Guid guidPesquisa, Guid guidUsuarioCacheExtrato, out StatusRetornoDTO statusRetornoDTO)
        {
            ...
            Logger.GravarLog(this.GetType().Name + " - guidPesquisa: " + guidPesquisa + ", guidUsuarioCacheExtrato = " + guidUsuarioCacheExtrato + ", XML envio.GetHashCode()=" + hashDadosPesquisa + ", estaEmCache=" + estaEmCache, envio);
            ...
            if (estaEmCache)
            {
                ...
                return (RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO>)cacheVO.RetornoPesquisa;
            }
            else
            {
                if (cacheVO != null)
                ...
                RetornoPesquisaComTotalizadorDTO<TRetornoDTO, TTotalizadorDTO> retorno = ExecutarPesquisa(out statusRetornoDTO, envio);

...
using AG = Redecard.PN.Extrato.Agentes.AntecipacaoRAVAG;
using Redecard.PN.Extrato.Modelo.Comum;
using System.Collections.Concurrent;

namespace Redecard.PN.Extrato.Negocio
{
    public class AntecipacaoRAVBLL : RegraDeNegocioBase
    {
        private static AntecipacaoRAVBLL _Instancia;
        public static AntecipacaoRAVBLL Instancia { get { return _Instancia ?? (_Instancia = new AntecipacaoRAVBLL()); } }
            ...
            out StatusRetornoDTO status)
        {
            using (Logger Log = Logger.IniciarLog("Consulta Totalizadores - Detalhe - WACA1332"))
            {
                try
                {
                    Log.GravarLog(EventoLog.ChamadaAgente, new { codigoBandeira, pvs, dataAntecipacao });
            ...
            out StatusRetornoDTO status)
        {
            ...
            var _dados = new ConcurrentQueue<RAVDetalheTotalizador>();
            ...
            Parallel.ForEach(_parametrosChamada, new ParallelOptions { MaxDegreeOfParallelism = 30 }, parametroChamada => 
            {
                try
                {
                    ...
                    DateTime _dataAntecipacao = parametroChamada.Item2;
            ...
            var _totalizador = new RAVDetalheTotalizador();
            status = _status;
            ...
            //Totaliza o Valor Disponível
            _totalizador.Totais.TotalValorDisponivel = _totalizadoresDia
                .Sum(totalizadorDia => totalizadorDia.Totais.TotalValorDisponivel);
            ...
            _totalizador.Valores = _totalizadoresDia
                ...
                .Select(grupoTotalizador => new RAVDetalheTotalizadorValor
                {
            ...
            out StatusRetornoDTO status)
        {
            using (Logger Log = Logger.IniciarLog("Consulta Dados - Detalhe - WACA1333"))
            {
                try
                {
                    Log.GravarLog(EventoLog.ChamadaAgente, new { codigoBandeira, pvs, dataAntecipacao, rechamada });
